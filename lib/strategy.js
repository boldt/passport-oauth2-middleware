...
OAuth2RefreshTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  if (authenticationURL && this._options.callbackParameter) {
    queryParams.push(encodeURIComponent(this._options.callbackParameter) + '=' + encodeURIComponent(req.url));
  }

  if (authenticationURL && this._options.scope) {
    queryParams.push('scope=' + this._options.scope);
  }

  if(queryParams.length > 0) {
    authenticationURL += '?' + queryParams.join('&');
  }
...
OAuth2RefreshTokenStrategy.prototype.getOAuth2StrategyCallback = function() {
  ...
  return function(accessToken, refreshToken, params, profile, done) {
    ...
    if(self._options.cert) {
        try {
            var cert = fs.readFileSync(self._options.cert);
            profile.jwt = jsonwebtoken.verify(accessToken, cert);
        ...
        try {
            profile.jwt = jsonwebtoken.decode(accessToken);
            profile.accessToken = accessToken;
            profile.refreshToken = refreshToken;
            debug('Token decode successful.');
        } catch(err) {
